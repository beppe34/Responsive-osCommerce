...
//        require_once('includes/classes/log.php');
\log::w("payson_payment_ipn_p.php: " . print_r($_POST,true));
	// Detect type of transaction
	if(isset($_POST['type']) && $_POST['type'] == 'TRANSFER') 
	{
		if(file_exists(DIR_FS_CATALOG . "includes/modules/payment/payson.php"))
        {
			include_once(DIR_FS_CATALOG . "includes/modules/payment/payson.php");
		...
		else
		{
			...
			error_log("Payson CCard Module not found: " . DIR_FS_CATALOG . "includes/modules/payment/payson.php", 0);
		}
	}
	...
	if($payson_payments->validate_IPN($req))
	{
		...
		tep_redirect(tep_href_link('checkout_process.php?token=' . $token.'&type='. $_POST['type'], '', 'SSL'));
