 ...
 */
\log::w("payson_payments_process: ");
if (strlen($_GET["token"]) > 0) {

    // Verify DATA if Request come back before IPN

    if (file_exists("/catalog/includes/modules/payment/payson.php")) {
        include_once("/catalog/includes/modules/payment/payson.php");
        $payson_payments = new payson;
    } else {
\log::w("Payson Module not found");
