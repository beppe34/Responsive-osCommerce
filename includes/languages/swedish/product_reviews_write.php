<?php
/*
  $Id$

  osCommerce, Open Source E-Commerce Solutions
  http://www.oscommerce.com

  Copyright (c) 2003 osCommerce

  Released under the GNU General Public License
*/

define('NAVBAR_TITLE', 'Recensioner');

define('SUB_TITLE_FROM', 'Från:');
define('SUB_TITLE_REVIEW', 'Din recension:');
define('SUB_TITLE_RATING', 'Betyg:');

define('TEXT_NO_HTML', '<small><font color="#ff0000"><strong>OBS:</strong></font></small>&nbsp;HTML översätts inte!');
define('TEXT_BAD', '<small><font color="#ff0000"><b>DÅLIG</b></font></small>');
define('TEXT_GOOD', '<small><font color="#ff0000"><b>BRA</b></font></small>');

define('TEXT_REVIEW_RECEIVED', 'Tack för din recension. Den har blivit skickad och väntar på godkännande innan den visas.');

define('TEXT_CLICK_TO_ENLARGE', 'Klicka för förstoring');
?>
