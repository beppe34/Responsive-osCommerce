  ...
  class category_tree {
    ...
    public function __construct($HideNegativeSortOrder=false) {
      ...
      if ( isset($_category_tree_data) ) {
        ...
        $hidesortorderwhereclause = $HideNegativeSortOrder?' and c.sort_order>-1 ':'';
        
        $categories_query = tep_db_query("select c.categories_id, c.parent_id, c.categories_image, cd.categories_name from " . TABLE_CATEGORIES . " c, " . TABLE_CATEGORIES_DESCRIPTION . " cd where c.categories_id = cd.categories_id and cd.language_id = '" . (int)$languages_id. "' " . $hidesortorderwhereclause . " order by c.parent_id, c.sort_order, cd.categories_name");
