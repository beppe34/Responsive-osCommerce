...
class klarna_partPayment extends KlarnaBase
{
    ...
    public function keys()
    {
        ...
        $keys = array(
            ...
            'MODULE_PAYMENT_PCKLARNA_LASTESTVERSION',
            'MODULE_PAYMENT_PCKLARNA_LIVEMODE'
        );
        foreach (KiTT::supportedCountries() as $country) {
            $keys[] = "MODULE_PAYMENT_PCKLARNA_EID_{$country}";
            $keys[] = "MODULE_PAYMENT_PCKLARNA_SECRET_{$country}";
            if (KiTT::countryLogic(KiTT::locale($country))->needAGB()) {
                $keys[] = "MODULE_PAYMENT_PCKLARNA_AGB_LINK_{$country}";
            }
        }
        $keys = array_merge(
            $keys,
