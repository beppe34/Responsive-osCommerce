...
class payson{
	...
	$this->api_endpoint     = "api.payson.se/1.0/";
        ...
        $this->test_agent_id   		= 	'4';
        $this->test_md5_key 		= 	'2acab30d-fe50-426f-90d7-8c60a7eb31d4';
				...
				if ($ignore == false && $size > 0) 
				{
	  				for ($i=0; $i<$size; $i++) 
	  				{
							...
							$tax = tep_get_tax_rate($this->tax_class_id)/100;
                                                        ...
                                                        if($class=="ot_coupon" || $class=="ot_gv"){
                                                            $price = -$price;
                                                        }
	    ...
	    tep_db_perform(
	    				TABLE_CONFIGURATION,
		    				array(
		    					...
		    					"configuration_value"       => "",
		    					"configuration_description" => "Your Payson API password (API Key)",
		    					"configuration_group_id"    => "6",
		    					"sort_order"                => "15",
		    					"date_added"                => "now()"
		    				)
	    ...
	    tep_db_perform(
	    				TABLE_CONFIGURATION,
		    				array(
		    					...
		    					"configuration_group_id"    => "6",
		    					"sort_order"                => "16",
		    					"set_function"              => "tep_cfg_select_option(array('ALL', 'CREDITCARD', 'BANK', 'SMS'), ",
		    					"date_added"                => "now()"
		    				)
	    				);
