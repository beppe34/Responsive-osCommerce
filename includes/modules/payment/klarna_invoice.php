...
class klarna_invoice extends KlarnaBase
{
    ...
    public function keys()
    {
        $keys = array(
            ...
            'MODULE_PAYMENT_KLARNA_LASTESTVERSION',
            'MODULE_PAYMENT_KLARNA_LIVEMODE'
        );
        foreach (KiTT::supportedCountries() as $country) {
            $keys[] = "MODULE_PAYMENT_KLARNA_EID_{$country}";
            $keys[] = "MODULE_PAYMENT_KLARNA_SECRET_{$country}";
            if (KiTT::countryLogic(KiTT::locale($country))->needAGB()) {
                $keys[] = "MODULE_PAYMENT_KLARNA_AGB_LINK_{$country}";
            }
        }
        $keys = array_merge(
            $keys,
