...
<tr valign="top">
	...
	echo '<br><ul><li><a href="' . tep_href_link('categories.php', 'pID=' . $VARS['product_id'] . '&action=new_product') . '" class="menuBoxContentLink">Edit this product</a></li>';
	echo '<li><a href="' . tep_href_link('stats_low_stock_attrib.php', '', 'NONSSL') . '" class="menuBoxContentLink">Go to the low stock report</a><br></li>';
	...
	//The same product can exist in differend categories.
	  ...
	  if (sizeof($raw_path_array)>0) {
		...
		foreach($path_array as $neverusedvariable) {
		  print '<li><a href="' . tep_href_link('categories.php', 'pID='.$VARS['product_id'].'&cPath='. $cpath_string_array[$curr_pos] , 'NONSSL') . '" class="menuBoxContentLink">Go to this product in '.$path_array[$curr_pos].'</a></li>';
		  $curr_pos++;
		}
	 }else{
