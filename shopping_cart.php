...
//++++ QT Pro: End Changed Code
      ...
      $products_name .= '<br>' . tep_draw_input_field('cart_quantity[]', $products[$i]['quantity'], 'style="width: 65px;" min="0"', 'number') . tep_draw_hidden_field('products_id[]', $products[$i]['id']) . ' ' . tep_draw_button(CART_BUTTON_UPDATE, 'fa fa-refresh', NULL, NULL, array ( params=>'data-toggle="tooltip" data-placement="top" title="' . IMAGE_BUTTON_UPDATE . '"'), 'btn-info btn-xs ') . ' ' . tep_draw_button(CART_BUTTON_REMOVE, 'fa fa-remove', tep_href_link('shopping_cart.php', 'products_id=' . $products[$i]['id'] . '&action=remove_product'), NULL, array ( params=>'data-toggle="tooltip" data-placement="top" title="' . IMAGE_BUTTON_REMOVE . '"'), 'btn-danger btn-xs');
